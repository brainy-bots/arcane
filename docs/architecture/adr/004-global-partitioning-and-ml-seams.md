...
- **Objective-guided partitioning (epic #293)** — capacity-factor-forced balance is replaced by the explicit objective `J = cut + α·Σ|S|^γ + β·open + μ·moves`. The partitioner now optimizes this cost model; balance emerges from the cost terms, not from a separate capacity constraint.
