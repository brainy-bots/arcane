...
- **Execute game logic or RPCs** between clusters — discrete game logic (attack hit, inventory) runs in SpacetimeDB reducers; ArcaneManager only handles assignment and topology. Optional RPCHandler (IN-05) is for non-game use only.
...
ArcaneManager is a long-running process. Its “API” is the **Manager WebSocket** (for clients) and the **SpacetimeDB write surface** (for assignment and topology). It does not expose a separate RPC API for Arcane Nodes; Arcane Nodes learn assignments and topology from SpacetimeDB subscriptions.
...
| ReplicationChannelManager (IN-06) | ArcaneManager must ensure replication is torn down before releasing a server. Neighbor list must reach IN-06 (via SpacetimeDB or a push). | If IN-06 gets neighbor list from SpacetimeDB, ArcaneManager only writes topology; if IN-06 gets a direct message, ArcaneManager must send it. |

---

## 8. Message Protocol

### 8.1 Manager connection (ArcaneManager ↔ client)

**Client → ArcaneManager (JSON over WebSocket):**
...
**ArcaneManager → Client:**

| Message | Format | When |
|---------|--------|------|
| CLUSTER_ASSIGN | `{ type, cluster_id, server_host, server_port, handoff_token? }` | After ArcaneManager has written the assignment to SpacetimeDB (first join or reassignment). |
...
### 8.2 ArcaneManager → ArcaneNode (notify “drop” or topology)

**Open:** Whether ArcaneManager sends an explicit message to a Arcane Node to “drop these players/entities at end of tick” is TBD. Alternatives:

- **A. SpacetimeDB only:** Arcane Nodes subscribe to assignments for their cluster_id. When ArcaneManager updates assignments (e.g. player P moved from B to A), server B’s subscription sees “P no longer in B” and server B drops P at end of tick. No explicit message from ArcaneManager to servers.
- **B. Control channel:** ArcaneManager sends a message (e.g. over TCP or a dedicated Redis topic) to each affected Arcane Node: “drop players [list] at end of tick.” Server acknowledges or does not; ArcaneManager does not block.
...
ArcaneManager writes **cluster topology** (which clusters exist, which are neighbors) to SpacetimeDB. ReplicationChannelManager (IN-06) on each Arcane Node subscribes to that topology (e.g. “neighbors of my cluster_id”) and opens/closes IReplicationChannel subscriptions accordingly. So ArcaneManager does not push directly to IN-06; it writes to SpacetimeDB and IN-06 reads via subscription.
...
|--------|------|--------|----------|
| arcane_manager_active_clusters | gauge | | Number of clusters that currently have at least one assigned player/entity. |
| arcane_manager_total_players | gauge | | Total players currently assigned (across all clusters). |
| arcane_manager_joins_total | counter | | PLAYER_JOIN requests processed. |
| arcane_manager_leaves_total | counter | | PLAYER_LEAVE processed. |
...
- **Explicit “drop” message to Arcane Nodes:** Use SpacetimeDB-only (servers see assignment change and drop) or add a control channel from ArcaneManager to Arcane Nodes for “drop at end of tick”? Recommendation: SpacetimeDB-only for MVP; add control channel only if ordering or latency requires it.
...
- **SpatialIndex ownership:** Is the spatial index a separate component (IN-03) with its own API, or inline in ArcaneManager? If separate, ArcaneManager depends on IN-03; if inline, IN-03 doc may describe the algorithm only and ArcaneManager implements it.

---

*Arcane Engine — IN-01 ArcaneManager — Confidential*
