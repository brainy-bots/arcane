...
**Arcane partitions by predicted interaction probability.** An AI clustering model (the live decision path is a global graph partition of the interaction graph, [`build_partition_decisions`](docs/architecture/adr/004-global-partitioning-and-ml-seams.md), with [`clustering-system-requirements`](docs/architecture/clustering-system-requirements.md) as the full system envelope; MVP implementation in place, ML production model planned) consumes live signals — player positions, party and guild relationships, recent interaction history, cross-cluster RPC load, game-layer hints from combat systems — and groups entities that *will* interact onto the same server. Spatial proximity is one signal among many, not the partitioning rule.
...
- [`docs/architecture/adr/004-global-partitioning-and-ml-seams.md`](docs/architecture/adr/004-global-partitioning-and-ml-seams.md) — the affinity-clustering decision path (global graph partition + ML seams), in detail.
